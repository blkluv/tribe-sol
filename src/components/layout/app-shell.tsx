...
function RightSidebar() {
  ...
  return (
    <aside className="hidden lg:block w-[350px] p-10 space-y-10 sticky top-0 h-screen overflow-y-auto bg-white">
      {/* Wallet / Profile Info */}
...
export function AppShell({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex min-h-screen bg-white">
      <SidebarNav />
      <div className="flex flex-1 justify-center bg-[#fcfcfc]">
        <main className="w-full max-w-[1000px] flex">
          <div className="flex-1 w-full max-w-[650px] border-x border-[#f0f0f0] min-h-screen pb-20 md:pb-0 bg-white">
            {children}
          </div>
          <RightSidebar />
        </main>
